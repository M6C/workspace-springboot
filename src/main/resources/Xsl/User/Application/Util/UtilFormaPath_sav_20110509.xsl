  ...
  <xsl:template name="replace-application-path">
    ...
    <xsl:choose>
      <xsl:when test="contains($text,'[') and contains($text,']')">
        ...
        <xsl:if test="$lPath = '' and $application != ''">
	        <xsl:variable name="lPath">
	          <xsl:call-template name="application-path">
	            <xsl:with-param name="applicationName" select="$application"/>
	            <xsl:with-param name="pathName" select="'Main'"/>
	          </xsl:call-template>
	        </xsl:variable>
    	</xsl:if>
      ...
      <xsl:otherwise>
	    <xsl:choose>
			<xsl:when test="$application != ''">
		        <!-- Si l'application est defini on ajoute le chemin de l'application -->
		    	<xsl:variable name="lPath">
					<xsl:call-template name="application-path">
						<xsl:with-param name="applicationName" select="$application"/>
						<xsl:with-param name="pathName" select="'Main'"/>
		        ...
		        <xsl:value-of select="concat(lPath, $text)"/>
			</xsl:when>
	      	<xsl:otherwise>
