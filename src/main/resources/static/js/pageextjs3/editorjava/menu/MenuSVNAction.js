...
function onMenuSVNTestCommit(item, event) {
	...
	Ext.Msg.confirm('Confirm', 'Test Commit ?', function(btn, text){
	    if (btn == 'yes'){
	    	...
	    	var requestUrl = ACTION_SERVLET_ROOT + '/action.servlet?event=AjaxSVNTestCommit';
  			Ext.Ajax.request({
  			   url: requestUrl,
  			   params: {},
...
function onMenuSVNCommit(item, event) {
	...
	Ext.Msg.prompt('Commit', 'Comment:', function(btn, text){
	    if (btn == 'ok'){
	    	...
	    	var requestUrl = ACTION_SERVLET_ROOT + '/action.servlet?event=AjaxSVNCommit';
  			Ext.Ajax.request({
  			   url: requestUrl,
...
function onMenuSVNImport(item, event) {
	...
	Ext.Msg.prompt('Commit', 'Comment:', function(btn, text){
	    if (btn == 'ok'){
	    	...
	    	console.info('onMenuSVNCommit');
	  		var project = Ext.getCmp('project').value;
	    	var requestUrl = ACTION_SERVLET_ROOT + '/action.servlet?event=AjaxSVNImport';
  			Ext.Ajax.request({
  			   url: requestUrl,
  			   params: {application:project,messageStr:text},
  			   success: function(result, request){
